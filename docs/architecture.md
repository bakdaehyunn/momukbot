...
- `search`: Naver Search API 호출과 블로그/로컬 컨텍스트 생성을 담당합니다. 블로그 결과는 `postdate`, 제목/요약 키워드, 방문 후기 표현, 영업시간 힌트, 광고 의심 표현으로 근거 우선순위를 계산하고 후보당 상위 근거를 제공합니다.
- `agent`: 사용자의 로컬 AI 에이전트를 호출합니다. v1은 `codex_cli`만 구현하며, 검증된 후보의 의도 적합도, 근거 품질, 메뉴군, 사용 상황, 리스트 다양성 그룹을 평가합니다.
- `core`: 요청 파싱, 프롬프트 생성, 추천 JSON 파싱, LLM 평가 기반 재정렬, 포맷팅을 담당합니다.
